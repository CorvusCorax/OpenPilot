...
static float directionIntegral = 0;
...
static void guidanceTask(void *parameters)
{
	...
	while (1) {
		...
		gyros[0] += attitudeRaw.gyros[0];
		gyros[1] += attitudeRaw.gyros[1];
		gyros[2] += attitudeRaw.gyros[2];
		accel_accum++;
		...
		accelData.X = gyros_ned[0];
		accelData.Y = gyros_ned[1];
		accelData.Z = gyros_ned[2];
...
printf("Position:	%i	%i	 %i	Desired:	%i	%i	%i\n",
	positionActual.East,positionActual.North,-positionActual.Down,
	positionDesired.East,positionDesired.North,-positionDesired.Down);
}
...
static void updateFixedDesiredAttitude()
{
	...
	directionError = RAD2DEG * (atan2f(velocityDesired.East,velocityDesired.North) - atan2f(velocityActual.East,velocityActual.North));
	if (directionError<-180.) directionError+=360.;
	if (directionError>180.) directionError-=360.;

printf("Course:	%i	Desired:	%i	Error:	%i\n",
	(int)(RAD2DEG * atan2f(velocityActual.East,velocityActual.North)),
	(int)(RAD2DEG * atan2f(velocityDesired.East,velocityDesired.North)),
	(int)(directionError)
	);

	directionIntegral = bound(directionIntegral + directionError * dT, 
			      -guidanceSettings.DirectionVelPID[GUIDANCESETTINGS_DIRECTIONVELPID_ILIMIT],
			      guidanceSettings.DirectionVelPID[GUIDANCESETTINGS_DIRECTIONVELPID_ILIMIT]);
	directionCommand = (directionError * guidanceSettings.DirectionVelPID[GUIDANCESETTINGS_DIRECTIONVELPID_KP] +
			directionIntegral * guidanceSettings.DirectionVelPID[GUIDANCESETTINGS_DIRECTIONVELPID_KI] -
			nedAccel.Z * guidanceSettings.DirectionVelPID[GUIDANCESETTINGS_DIRECTIONVELPID_KD]);
	
	stabDesired.Roll = bound( directionCommand, -guidanceSettings.MaxRollPitch, guidanceSettings.MaxRollPitch );
...
printf("Roll angle:	%i	Yaw rate:	%f\n",
	(int)stabDesired.Roll,
	stabDesired.Yaw);
...
printf("Speed:	%i	Desired:	%i	Error:	%i\n",
	(int)speedActual,
	(int)speedDesired,
	(int)speedError
	);

printf("Acceleration:	%i	Desired:	%i	Error:	%i\n",
	(int)attitudeRaw.accels[0],
	(int)accelDesired,
	(int)accelError
	);

printf("Desired pitch angle: %i\n",(int)stabDesired.Pitch);

	// Compute desired down command
	powerDesired = bound(
				guidanceSettings.PowerP[GUIDANCESETTINGS_POWERP_KP] * (accelDesired * speedActual - GEE * velocityDesired.Down),
				-guidanceSettings.PowerP[GUIDANCESETTINGS_POWERP_MAX],
				guidanceSettings.PowerP[GUIDANCESETTINGS_POWERP_MAX]);
	powerError = powerDesired - powerActual;
	powerIntegral =	bound(powerIntegral + powerError * dT, 
			      -guidanceSettings.PowerVelPI[GUIDANCESETTINGS_POWERVELPI_ILIMIT],
			      guidanceSettings.PowerVelPI[GUIDANCESETTINGS_POWERVELPI_ILIMIT]);	
	powerCommand = 0.5+(powerError * guidanceSettings.PowerVelPI[GUIDANCESETTINGS_POWERVELPI_KP] +
		       powerIntegral * guidanceSettings.PowerVelPI[GUIDANCESETTINGS_POWERVELPI_KI]);
	
	stabDesired.Throttle = bound(powerCommand, 0, 1);

printf("Power:	%i	Desired:	%i	Error:	%i\n",
	(int)powerActual,
	(int)powerDesired,
	(int)powerError
	);

printf("Power calculation: acceleration: %i , climb: %i\n",
	(int)(attitudeRaw.accels[0] * speedActual),
	(int)(-GEE * velocityActual.Down));
printf("Power desired calculation: acceleration: %i , climb: %i\n",
	(int)(accelDesired * speedActual),
	(int)(-GEE * velocityDesired.Down));

printf("Desired throttle: %i\n",(int)(stabDesired.Throttle*100.));
